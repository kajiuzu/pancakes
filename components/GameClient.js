  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      class MainScene extends Phaser.Scene {
        ...
        spawn() { const thick = Math.random() < Math.min(0.7, 0.25 + this.state.stack.length * 0.03); const endScale = thick ? 1.0 : 0.88; const diff = Math.min(1.7, 1 + this.state.stack.length * 0.04); const targetX = this.plateX + (Math.random() * 120 - 60); const apexLift = 130 + Math.random() * 60; this.flying = { startX: this.chefX + (Math.random() * 90 - 45), startY: this.chefY + (Math.random() * 20 - 10), targetX, targetY: this.baseY - 30, progress: 0, speed: 0.50 * diff, arc: apexLift, baseW: 112, baseH: 24, endScale, x: this.chefX, y: this.chefY, w: 42, h: 10, thick }; }
        ...
        update(_, dtMs) {
          ...
          if (this.state.running) {
            ...
            const p = this.flying; p.progress = Math.min(1.25, p.progress + p.speed * dt); const t = p.progress; const curve = 4 * t * (1 - t); p.x = p.startX + (p.targetX - p.startX) * t; p.y = p.startY + (p.targetY - p.startY) * t - p.arc * curve; const scale = 0.36 + Math.min(1, t) * (p.endScale - 0.36); p.w = p.baseW * scale; p.h = p.baseH * scale;
            ...
            if (p.progress > 1.2 || p.y > 620) return this.end("キャッチ失敗！パンケーキを落とした");
          }
